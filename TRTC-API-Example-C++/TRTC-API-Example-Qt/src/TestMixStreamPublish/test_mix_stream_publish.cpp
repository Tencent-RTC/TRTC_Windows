...
TestMixStreamPublish::TestMixStreamPublish(QWidget* parent)
    : BaseDialog(parent),
    ui_test_mix_stream_publish_(new Ui::TestMixStreamPublishDialog) {
    trtc_transcoding_config.appId = APPID;
    trtc_transcoding_config.bizId = BIZID;
...
void TestMixStreamPublish::startPureAudioTemplate() {
    ...
    // If 0 is passed in, the backend will calculate a bitrate based on videoWidth
    // and videoHeight.
    ...
    std::string streamid_str =
        ui_test_mix_stream_publish_->streamIdLineEt->text().toStdString();
    trtc_transcoding_config.streamId = streamid_str.c_str();
    std::string roomid_str =
        std::to_string(RoomInfoHolder::GetInstance().getMainRoomId());
...
void TestMixStreamPublish::startPresetLayoutTemplate() {
    ...
    for (int current_pos = 1; current_pos < current_mix_size; current_pos++) {
        ...
        mix_users_array[current_pos].rect.left =
            (current_pos - 1) * remote_item_width;
        mix_users_array[current_pos].rect.top =
            video_height_ - 4 * remote_item_width / 3;
        mix_users_array[current_pos].rect.bottom =
            mix_users_array[current_pos].rect.top + remote_item_height;
        mix_users_array[current_pos].rect.right =
            mix_users_array[current_pos].rect.left + remote_item_width;
    }
...
void TestMixStreamPublish::startManualTemplate() {
    ...
    int max_mix_size =
        remote_item_width == 0 ? 1 : (video_width_ / remote_item_width + 1);

    current_mix_size =
        mix_usersarray_size > max_mix_size ? max_mix_size : mix_usersarray_size;
    ...
    for (auto remote_info : remote_userinfos_) {
        ...
        if (remote_info->video_available_) {
            mix_users_array[current_pos].inputType =
                liteav::TRTCMixInputTypePureVideo;
        }

        if (remote_info->audio_available_) {
            mix_users_array[current_pos].inputType =
                liteav::TRTCMixInputTypePureAudio;
        }

        if (remote_info->video_available_ && remote_info->audio_available_) {
            mix_users_array[current_pos].inputType =
                liteav::TRTCMixInputTypeAudioVideo;
        }
        ...
        mix_users_array[current_pos].rect.left =
            (current_pos - 1) * remote_item_width;
        mix_users_array[current_pos].rect.top =
            (current_pos - 1) * (video_height_ / max_mix_size - 1);
        mix_users_array[current_pos].rect.bottom =
            mix_users_array[current_pos].rect.top + remote_item_height;
        mix_users_array[current_pos].rect.right =
            mix_users_array[current_pos].rect.left + remote_item_width;
...
void TestMixStreamPublish::showEvent(QShowEvent* event) {
    ui_test_mix_stream_publish_->streamIdLineEt->setText(QString::fromStdString(
        RoomInfoHolder::GetInstance().getMixTranscodingStreamId()));
    QDialog::showEvent(event);
}

void TestMixStreamPublish::updatePublishButtonStatus() {
    updateDynamicTextUI();
    ui_test_mix_stream_publish_->startMixStreamPublishBt->setEnabled(
        isStartMixStreamBtAvailable());
}
...
void TestMixStreamPublish::onSetMixTranscodingConfig(int errCode,
    const char* errMsg) {
    if (errCode != 0) {
        QMessageBox::warning(this, "SetMixTranscodingConfig() failed", errMsg,
            QMessageBox::Ok);
        started_transcoding_ = false;
    }
    updatePublishButtonStatus();
...
void TestMixStreamPublish::onUserVideoAvailable(const char* userId,
    bool available) {
    ...
    else if (iter != remote_userinfos_.end()) {
    ...
    if (mix_config_mode_ == liteav::TRTCTranscodingConfigMode_Manual &&
        started_transcoding_) {
        updateTranscodingConfig();
    }
}

void TestMixStreamPublish::onUserAudioAvailable(const char* userId,
    bool available) {
    std::vector<RemoteUserInfo*>::const_iterator iter = remote_userinfos_.begin();
    std::vector<RemoteUserInfo*>::const_iterator find_iter =
        remote_userinfos_.end();
    while (iter != remote_userinfos_.end()) {
        if (strcmp((*iter)->user_id_.c_str(), userId) == 0) {
            break;
        }
...
bool TestMixStreamPublish::isStartMixStreamBtAvailable() {
    return (!ui_test_mix_stream_publish_->streamIdLineEt->text().isEmpty()) ||
        started_transcoding_;
}

void TestMixStreamPublish::initUI() {
    config_mode_button_group.addButton(
        ui_test_mix_stream_publish_->radioButtonManual, 0);
    config_mode_button_group.addButton(
        ui_test_mix_stream_publish_->radioButtonPresetLayout, 1);
    config_mode_button_group.addButton(
        ui_test_mix_stream_publish_->radioButtonScreenSharing, 2);
    config_mode_button_group.addButton(
        ui_test_mix_stream_publish_->radioButtonPureAudio, 3);
    ui_test_mix_stream_publish_->radioButtonManual->setChecked(true);
    connect(ui_test_mix_stream_publish_->radioButtonManual, SIGNAL(clicked(bool)),
        this, SLOT(on_config_mode_checked_change()));
    connect(ui_test_mix_stream_publish_->radioButtonPresetLayout,
        SIGNAL(clicked(bool)), this, SLOT(on_config_mode_checked_change()));
    connect(ui_test_mix_stream_publish_->radioButtonScreenSharing,
        SIGNAL(clicked(bool)), this, SLOT(on_config_mode_checked_change()));
    connect(ui_test_mix_stream_publish_->radioButtonPureAudio,
        SIGNAL(clicked(bool)), this, SLOT(on_config_mode_checked_change()));
...
void TestMixStreamPublish::on_streamIdLineEt_textChanged(
...
void TestMixStreamPublish::updateTranscodingConfig() {
    if (mix_config_mode_ ==
        liteav::TRTCTranscodingConfigMode_Template_PureAudio) {
        startPureAudioTemplate();
    }

    if (mix_config_mode_ ==
        liteav::TRTCTranscodingConfigMode_Template_ScreenSharing) {
        startScreenSharingTemplate();
    }

    if (mix_config_mode_ ==
        liteav::TRTCTranscodingConfigMode_Template_PresetLayout) {
...
bool TestMixStreamPublish::getTranscodingConfig() {
    ...
    video_height_ =
        ui_test_mix_stream_publish_->videoHeightLineEt->text().toInt();
    video_bitrate_ =
        ui_test_mix_stream_publish_->LineEtVideoBitrate->text().toInt();
    video_framerate_ =
        ui_test_mix_stream_publish_->videoFramerateEt->text().toInt();
    video_gop_ = ui_test_mix_stream_publish_->videoGopEt->text().toInt();
    background_color_ =
        ui_test_mix_stream_publish_->videoBackgrioundColorComB->currentIndex() ==
        0
        ? 0x000000
        : 0x0000FF;
    std::string backgroud_image_str =
        ui_test_mix_stream_publish_->videoBackgroundImageEt->text().toStdString();
    background_imag_ = backgroud_image_str;
    audio_samplerate_ = TestMixStreamPublish::kAudioSampleRate
        [ui_test_mix_stream_publish_->audioSampleRateComB->currentIndex()];
    audio_bitrate_ = ui_test_mix_stream_publish_->audioBitrateEt->text().toInt();
    audio_channels_ =
        (ui_test_mix_stream_publish_->audioChannelsComB->currentIndex() == 0) ? 1
        : 2;

    if (mix_config_mode_ == liteav::TRTCTranscodingConfigMode_Unknown) {
        QMessageBox::warning(this, "Failed to publish mixed streams",
            "Select a layout mode.", QMessageBox::Ok);
        return false;
    }

    if (audio_bitrate_ < 32 || audio_bitrate_ > 192) {
        QMessageBox::warning(this, "Failed to publish mixed streams",
            "The audio bitrate must be in the range of [32, 192].",
            QMessageBox::Ok);
        return false;
    }
    ...
    if (video_gop_ < 1 || video_gop_ > 8) {
        QMessageBox::warning(
            this, "Failed to publish mixed streams",
            "The keyframe interval (GOP) must be in the range of [1, 8].",
            QMessageBox::Ok);
        return false;
    }
    ...
    if (video_framerate_ <= 0 || video_framerate_ > 30) {
        QMessageBox::warning(this, "Failed to publish mixed streams",
            "The frame rate must be in the range of (0, 30].",
            QMessageBox::Ok);
        return false;
    }
...
void TestMixStreamPublish::updateDynamicTextUI() {
    if (started_transcoding_) {
        ui_test_mix_stream_publish_->startMixStreamPublishBt->setText(
            tr("停止发布"));
    }
    else {
        ui_test_mix_stream_publish_->startMixStreamPublishBt->setText(
