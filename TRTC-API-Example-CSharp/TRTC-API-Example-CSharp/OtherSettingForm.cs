...
namespace TRTCCSharpDemo
{
    public partial class OtherSettingForm : Form
    {
        ...
        private volatile bool mStartSubCustomCaptureVideo;
        private volatile bool mStartSubCustomCaptureAudio;
        ...
        private Thread mVideoSubCustomThread;
        private Thread mAudioSubCustomThread;
        ...
        private void OnDisposed(object sender, EventArgs e)
        {
            ...
            if (mTRTCCloud != null && mMainForm != null)
            {
                ...
                mStartSubCustomCaptureAudio = false;
                mStartSubCustomCaptureVideo = false;
                ...
                if (mVideoSubCustomThread != null)
                {
                    mVideoSubCustomThread.Join();
                    mVideoSubCustomThread.DisableComObjectEagerCleanup();
                    mVideoSubCustomThread = null;
                }
                if (mAudioSubCustomThread != null)
                {
                    mAudioSubCustomThread.Join();
                    mAudioSubCustomThread.DisableComObjectEagerCleanup();
                    mAudioSubCustomThread = null;
                }
        ...
        private void StartCustomCaptureVideo(string path, uint width, uint height)
        {
            ...
            mTRTCCloud.enableCustomVideoCapture(TRTCVideoStreamType.TRTCVideoStreamTypeBig,true);
        ...
        private void StartSubCustomCaptureVideo(string path, uint width, uint height)
        {
            ...
            mStartSubCustomCaptureVideo = true;
            mTRTCCloud.enableCustomVideoCapture(TRTCVideoStreamType.TRTCVideoStreamTypeSub, true);

            if (mVideoSubCustomThread == null)
            {
                mVideoSubCustomThread = new Thread(() =>
                {
                    while (mStartSubCustomCaptureVideo)
                    {
                        SendSubCustomVideoFrame();
                        Thread.Sleep(66);
                    }
                })
                { IsBackground = true };
                mVideoSubCustomThread.Start();
            }
        }

        public void StopSubCustomCaptureVideo()
        {
            if(mStartSubCustomCaptureVideo)
            {
                mStartSubCustomCaptureVideo = false;
                mTRTCCloud.enableCustomVideoCapture(TRTCVideoStreamType.TRTCVideoStreamTypeSub, false);
                if (mVideoSubCustomThread != null)
                {
                    mVideoSubCustomThread.Join();
                    mVideoSubCustomThread.DisableComObjectEagerCleanup();
        ...
        public void SendSubCustomVideoFrame()
        {
            if (!mStartSubCustomCaptureVideo) return;
            try
            {
                using (FileStream fs = File.OpenRead(mVideoFilePath))
                {
                    ...
                    frame.length = bufSize;
                    mTRTCCloud.sendCustomVideoData(TRTCVideoStreamType.TRTCVideoStreamTypeSub, frame);
                }
            }
            catch (Exception e)
            {
        ...
        public void StartSubCustomCaptureAudio(string path, uint samplerate, uint channel)
        {
            mAudioFilePath = path;
            mAudioSamplerate = samplerate;
            mAudioChannel = channel;
            ...
            mStartSubCustomCaptureAudio = true;
            mTRTCCloud.enableMixExternalAudioFrame(true,true);

            if (mAudioSubCustomThread == null)
            {
                mAudioSubCustomThread = new Thread(() =>
                {
                    while (mStartSubCustomCaptureAudio)
                    {
                        SendSubCustomAudioFrame();
                        Thread.Sleep(20);
                    }
                })
                { IsBackground = true };
                mAudioSubCustomThread.Start();
            }
        }

        public void SendSubCustomAudioFrame()
        {
            if (!mStartSubCustomCaptureAudio) return;
            try
            {
                using (FileStream fs = File.OpenRead(mAudioFilePath))
                {
                    ...
                    frame.data = mAudioBuffer;
                    frame.sampleRate = mAudioSamplerate;
                    mTRTCCloud.mixExternalAudioFrame(frame);
                }
            }
            catch (Exception e)
            {
        ...
        public void StopSubCustomCaptureAudio()
        {
            if (mStartSubCustomCaptureAudio)
            {
                mStartSubCustomCaptureAudio = false;
                mTRTCCloud.enableMixExternalAudioFrame(false, false);
                if (mAudioSubCustomThread != null)
                {
                    mAudioSubCustomThread.Join();
                    mAudioSubCustomThread.DisableComObjectEagerCleanup();
        ...
        public void SendCustomVideoFrame()
        {
            ...
            try
            {
                using (FileStream fs = File.OpenRead(mVideoFilePath))
                {
                    ...
                    frame.height = mVideoHeight;
                    frame.length = bufSize;
                    mTRTCCloud.sendCustomVideoData(TRTCVideoStreamType.TRTCVideoStreamTypeBig,frame);
                }
            }
            catch (Exception e)
            {
